            ...
            if (!empty($answers)) {
                foreach ($answers as $q_id => $data) {
                    ...
                    $wpdb->update(
                        ...
                        ['submission_id' => $submission_id, 'question_id' => $q_id]
                    );

                    if ($is_correct == 1) $correct_answers++;
                }
            }
            
            // Đếm lại tổng số câu đúng từ DB để đảm bảo chính xác
...
function render_grader_dashboard_tables() {
    ...
    if ($pending_submissions) {
        ...
        foreach ($pending_submissions as $sub) {
            ...
            echo '<td><strong>' . esc_html($sub->submitter_name) . '</strong></td>'; // THÊM TÊN THÍ SINH
            echo '<td>' . date('d/m/Y H:i', strtotime($sub->end_time)) . '</td>';
            echo '<td><a href="' . esc_url($grading_url) . '" class="button">Chấm bài</a> <a href="' . esc_url($delete_url) . '" class="button delete-button" onclick="return confirm(\'Bạn có chắc chắn muốn xóa vĩnh viễn bài thi này không?\');">Xóa</a></td>';
            echo '</tr>';
        }
        echo '</tbody></table>';
    } else {
    ...
    echo '<h2 style="margin-top: 40px;">Lịch sử chấm bài</h2>';
     if ($graded_submissions) {
        ...
        foreach ($graded_submissions as $sub) {
            ...
            echo '<td><strong>' . esc_html($sub->submitter_name) . '</strong></td>'; // THÊM TÊN THÍ SINH
            echo '<td>' . date('d/m/Y H:i', strtotime($sub->end_time)) . '</td>';
            echo '<td><strong><a href="' . esc_url($review_url) . '">' . intval($sub->score) . '/' . $total_questions_for_sub . '</a></strong></td>';
            echo '<td><a href="' . esc_url($review_url) . '" class="button review-button">Xem lại</a> <a href="' . esc_url($delete_url) . '" class="button delete-button" onclick="return confirm(\'Bạn có chắc chắn muốn xóa vĩnh viễn bài thi này không?\');">Xóa</a></td>';
            echo '</tr>';
        }
        echo '</tbody></table>';
    } else {
...
function render_single_submission_grading_form($submission_id) {
    ...
    echo '<h3>Thí sinh: ' . esc_html($submission->submitter_name) . '</h3>'; // Hiển thị tên thí sinh
    if ($is_graded) echo '<em>(Đây là bài thi đã được chấm. Bạn đang ở chế độ xem lại.)</em>';

    // Form chỉ hiển thị khi bài chưa được chấm
    ...
    foreach ($answers as $answer) {
        ...
        if ($question) {
            ...
            echo '<input type="hidden" name="answers[' . $answer->question_id . '][type]" value="' . esc_attr($loai_cau_hoi) . '">';
            ...
            if ($loai_cau_hoi === 'trac_nghiem') {
                ...
                echo '</div>';
                if (!$is_graded) {
                    echo '<div class="grading-tick"><label><input type="checkbox" name="answers[' . $answer->question_id . '][answer]" value="1"> <strong>Đánh dấu câu trả lời này là Đúng</strong></label></div>';
                }
            }
            echo '</div>';
        }
    ...
    if (!$is_graded) {
        echo '<button type="submit" class="finish-button">Hoàn tất chấm bài</button>';
        echo '</form>';
    }
}